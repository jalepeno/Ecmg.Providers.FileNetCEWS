...
namespace Documents.Providers.FileNetCEWS
{
    public class MtomMessageEncoder : MessageEncoder
    {
        ...
        public override Message ReadMessage(ArraySegment<byte> buffer, BufferManager bufferManager, string contentType)
        {
            ...
            {
                var message = ReadMessage(stream, 16024, contentType);
        ...
        private static IEnumerable<HttpContent> GetMultipartContent(Stream stream, string contentType)
        {
            ...
            //IEnumerable<HttpContent> response = null;
            //var result = content.ReadAsStreamAsync().GetAwaiter().GetResult();
            //return response;

            var result = content.ReadAsMultipartAsync().GetAwaiter().GetResult();
